...
## Port wave: epool + anatomy corrections (2026-08-26)

Benedikt green-lit porting the m=32 menu (open parked for an M3/M4
revisit). Status:

**epool ported** (`crates/flock-core/src/epool.rs`, ~230 lines + call-site
branches): bounded-tail hetero chunk queue — the current rayon pool and a
2-thread background-QoS (E-core) helper pool drain one shared atomic
index; an E-core only ever holds its most recent chunk, so the barrier
waits on at most one E-sized chunk per helper. Wired into the zc round-2
fused fold and the rounds-3+ fused tail (engage at ≥2^20 outputs).
FLOCK_NO_EPOOL=1 restores the incumbent rayon path exactly. Bit-identical
by construction (exact XOR merge) and by test at an engaging shape
(m=26 vs the serial oracle + tail-vs-unfused reference); full roundtrips
pass on all three circuits. Smoke at m=32: r2 47.5-50.1 (their timed
49.4 — parity BEFORE the anchor+delta port), tail 54.9-62.2. Paired
kill-switch A/B running.

**"Lincheck stripe-fold reuse" CLOSED — already ours.** Peer anatomy
inverted the item: the mechanism is zc round 1's C-branch reusing the
lincheck stripe buffer, which our tree has had since the ST campaign
(StripeC + [zc-r1] stripe fold, §stripe of the writeup). Both trees are
architecturally identical here; the ~3-5ms lincheck bucket delta has no
portable mechanism behind it. Menu item retired.

**Peer timed-path correction**: their prove_fast_timed skips THEIR
r1 stripe fast path, so every cross-tree round-1 comparison today used an
upper bound on their r1 cost; their scored path is faster. r2/commit/
lincheck comparisons unaffected.

**r2 anchor+delta + cascade anatomy received** (full mechanism): r2
stores anchor=folded even element + delta=raw unfolded byte-XOR of the
row pair (GF(2)-linearity defers the odd fold); r3 composes ρ₁ into a
32KiB byte table once, combine becomes lookup+XOR instead of two muls per
output. Lookahead defers r3's message as a symbolic quadratic in ρ₁
(coefficients computed register-resident during r2); cascade2/3(/4/5)
repeat the induction at rounds 5+6, 7+8(, 9+10, 11+12), fusing each pair
into one composed double-fold. Their m==32 gate is a validated-shapes
allowlist, not structural (real bounds: n_mlv thresholds 6/7/8/10/12).
Portability check PASSES: the cascade's fixed-challenge slots
(r[k_skip+1], +3, +5) sit in the protocol-pinned friendly ranges, which
our protocol shares. Port next.
