...
// test_zc_cpustyle_gpu). Compare to warp5 (~3.66 ms @ m=29) and CPU (~5 ms).
// Build: make bench_zc_cpustyle ; run: ./bench_zc_cpustyle [m] [iters]
#include <cstdio>
#include <cstdint>
#include <cstdlib>
#include <vector>
...
static void run_one(int m, int iters){
    ...
    CK(cudaMalloc(&d_eq,n_out*sizeof(F128))); CK(cudaMalloc(&d_conv,(size_t)16*256*sizeof(F128)));
    ...
    fill_f128<<<(unsigned)((16*256+255)/256),256>>>(d_conv,16*256);
    CK(cudaDeviceSynchronize());
    F128 scale{0x123,0x456};

    cudaEvent_t e0,e1; cudaEventCreate(&e0); cudaEventCreate(&e1);
    ZC_CS_LAUNCH(d_a,d_b,d_c,d_eq,n_out,d_conv,scale,d_ab,d_c_out); CK(cudaDeviceSynchronize());
    float t=0;
    ...
    t/=iters;
    printf("m=%2d n_out=%8lld rows=%10lld | cpustyle %8.3f ms\n", m, n_out, n_out*128, t);
    cudaFree(d_a);cudaFree(d_b);cudaFree(d_c);cudaFree(d_eq);cudaFree(d_conv);cudaFree(d_ab);cudaFree(d_c_out);
}
