...
static void zerocheck_phase(F128* da, F128* db, F128* dc, int m, FsChallenger& ch, Phase& ph,
    ...
    auto do_msg = [&](int r_from) -> ChF128 {
        ...
        launch_zt_msg(cA, cB, d_eq, half, d_p1, d_pinf, d_m1, d_minf);
    ...
    auto do_fused_eq = [&](const F128* d_eqbuf, long long op, F128 rho) -> ChF128 {
        launch_zt_fold_msg(cA, cB, nA, nB, d_eqbuf, op, rho, d_p1, d_pinf, d_m1, d_minf);
    ...
    { long long L = len;
      for (int i = 0; i < n_tail; i++) {
          long long op = L / 4;
          launch_eq_halve_scale(eprev, escr, op, sc[i]);                 // eq(r[8+i..m]) from eq(r[7+i..m])
    ...
    static bool _pr=false; if(!_pr){_pr=true; printf("  [zc] r1-eqbuild %.2f  round1-kernel %.2f  round2(lag+fold) %.2f  msg1 %.2f  tail(22) %.2f  final %.2f ms\n", t_r1eq,t_r1,t_r2,t_msg1,t_tail,t_fin);}
