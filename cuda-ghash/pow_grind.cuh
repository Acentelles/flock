...
__device__ __forceinline__ uint32_t pow_bswap32(uint32_t x) {
    return ((x & 0x000000ffu) << 24) | ((x & 0x0000ff00u) << 8) |
           ((x & 0x00ff0000u) >> 8) | ((x & 0xff000000u) >> 24);
}

__device__ __forceinline__ bool pow_has_leading_zero_bits(const uint32_t hash[8], uint32_t bits) {
...
__global__ void pow_scan_sha256(const uint8_t* __restrict__ state_digest, uint64_t start,
...
#pragma unroll
        ...
        words[8] = pow_bswap32((uint32_t)nonce);
        words[9] = pow_bswap32((uint32_t)(nonce >> 32));
        words[10] = 0x80000000u;
#pragma unroll
        for (int i = 11; i < 15; i++) words[i] = 0;
        words[15] = 40 * 8;
        sha256_compress_words(hash, words);
        if (pow_has_leading_zero_bits(hash, bits)) atomicMin(best, (unsigned long long)nonce);
    }
}
...
inline cudaError_t grind_pow_device(FsChallenger& challenger, uint32_t bits, uint64_t& nonce) {
    ...
    if (bits == 0) {
        ...
        for (;;) {
            ...
            if (err != cudaSuccess) return err;
            pow_scan_sha256<<<1024, 256>>>(d_state_digest, start, chunk, bits, d_best);
