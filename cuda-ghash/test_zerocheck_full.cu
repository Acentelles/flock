...
int main(int argc, char** argv){
    ...
    ZcSha* d_state; F128 *d_rho,*d_rhos,*d_m1log,*d_milog,*d_eqall;
    int n_mlv_alloc = m - 6;
    ...
    CK(cudaMalloc(&d_eqall,n_out*sizeof(F128)));
    ...
    long long len=n_out;
    // round-2 message: eq = build_eq(r[7..m]).
    auto msg_round = [&](const std::vector<F128>& r_rest, F128& m1, F128& mi){
        long long half=len/2;
        std::vector<F128> eq=build_eq(r_rest);
        CK(cudaMemcpy(d_eq, eq.data(), half*sizeof(F128), cudaMemcpyHostToDevice));
        launch_zt_msg(cA,cB,d_eq,half,d_p1,d_pinf,d_m1d,d_mid);
        CK(cudaMemcpy(&m1,d_m1d,sizeof(F128),cudaMemcpyDeviceToHost));
        CK(cudaMemcpy(&mi,d_mid,sizeof(F128),cudaMemcpyDeviceToHost));
    };
    F128 m1,mi;
    { std::vector<F128> r7(r.begin()+7, r.end()); msg_round(r7,m1,mi); }
    ...
    int n_tail = n_mlv - 1;
    // INCREMENTAL eq: build eq(r[7..m]) ONCE, then halve+scale to derive each tail round's
    // eq (nested: eq(r[j+1..m])[y]=eq(r[j..m])[2y]·(1+r[j])^{-1}) — no per-round rebuild.
    std::vector<long long> eqoff(n_tail);
    { std::vector<F128> base=build_eq(std::vector<F128>(r.begin()+7, r.end()));
      CK(cudaMemcpy(d_eq, base.data(), base.size()*sizeof(F128), cudaMemcpyHostToDevice));
      const F128 ONE{1,0}; long long oacc=0, L=len; const F128* prev=d_eq;
      for(int i=0;i<n_tail;i++){ long long op=L/4;
          F128 sc=f128_inv_host(f128_add_hd(ONE, r[7+i]));   // (1+r[7+i])^{-1}
          launch_eq_halve_scale(prev, d_eqall+oacc, op, sc);
          eqoff[i]=oacc; prev=d_eqall+oacc; oacc+=op; L/=2; } }
    ...
    { long long L=len;
      for(int i=0;i<n_tail;i++){ long long op=L/4;
        launch_zt_fold_msg_dev(cA,cB,nA,nB,d_eqall+eqoff[i],op,d_rho,d_p1,d_pinf,d_m1d,d_mid);
