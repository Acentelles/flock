...
\subsection{The boundary join: bandwidth and gathers share the pool}
\label{sec:boundaryjoin}

\begin{optsummary}
\item[Baseline:] At the direct open's level-1 boundary, the composed witness fold (one streaming pass over the full witness, DRAM-bandwidth-bound) and the residual-basis build (byte-table folds, 16 L1 gathers per element, compute-bound) ran back-to-back: $4.5 + 5.6\,$ms.
\item[Improvement:] They are data-independent at the boundary --- \code{rayon::join} runs them concurrently on the shared pool.
\item[Mechanism:] The same compute-under-bandwidth scheduling as the round-1 AB hoist (\S\ref{sec:abhoist}): a bandwidth-bound pass leaves core issue slots idle that a gather/compute-bound task can fill. No transcript interaction in either arm, so values and proof bytes are unchanged.
\item[Result:] Boundary pair $10.1\to7.3$--$8.4\,$ms; the open's initial-sumcheck stage $11.3\to8.5$--$9.8\,$ms at $m{=}32$ 8T. Certified jointly with \S\ref{sec:blockedtranspose}.
\end{optsummary}
