...
shape. Every \emph{numbered} section that follows is one verified
optimization, presented with its mechanism and its paired measurement.}
...
\paragraph{The rounds-3+ tail: fusing away a memory pass.}
The tail rounds (3 and beyond) apply the same treatment plus one thing round~2
had no opportunity for. The incumbent made \emph{two} passes per worker chunk:
...
\section{PCS open: composing the block scalar into the fold table}
...
\section{Witness generation: streaming full-word publication}
\label{sec:streamwit}
...
\section{Lincheck fold: one word load per stripe, two stripes per pass}
\label{sec:lcfold}
...
\section{Merkle BLAKE3: two transposed four-wide states in flight}
...
\S\ref{sec:qres} (round 2) & representation & round 2: $-13.0\%$ & $8/8$\\
\S\ref{sec:qres} (rounds 3+) & pass fusion & rounds 3+: $-31.8\%$ & $8/8$\\
