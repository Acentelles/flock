...
\emph{This section is front matter: shared definitions and the benchmark
shape. The numbered sections group the work by prover phase, in pipeline
order; each \emph{subsection} is one verified optimization, presented with
its mechanism and its paired measurement.}
...
weight $x^K$ is factored so one piece stays a table image while the $x^2$
...
being the generator rather than on \eqref{eq:friendly} itself). The rule
separating the directions: a table wins when one fixed map is applied to a
stream of arbitrary inputs (the load-port-bound drains); a fold at the value
wins when the input is a single accumulator or the map itself is being
constructed. Folds at freshly \emph{sampled} challenges (rounds two and
onward) get no such shortcut --- there a fold is an ordinary multiply.
...
Everything downstream consumes the bit-packed witness; its builder is the
first timed phase.
...
aligned) is reserved and overwritten once the final state is known. The
fixed 56-G compression schedule is also fully unrolled with literal
state/message indices, exposing the whole dependency graph to register
allocation. Bit-identical through the driver (tested against the OR
builder on real and padding slots, both values of the prefix carry bit).
...
$\approx$400 lines and not taken.
...
fuses to a single \code{EOR3} under the SHA3 feature: $\approx$9 loads per
stripe, same XOR multiset, bit-identical. Measured, paired:
...
as the slot-multiply path --- so the transcript is bit-identical (unit
equivalence test; verification of full proofs unchanged).
...
\paragraph{Negative space for the open.}
...
crate path; byte-identical by an equivalence test over all batched message
sizes, counts, and flag combinations.
...
provenance tag (derived independently at the buffer's give site from the
R1CS constants and at its take site from the encoder's own constants ---
no plumbing carries it) marks a pooled buffer as holding exactly a previous
prove's output of this layout; any other pool custody clears the tag. On a
tagged hit the builder skips the constant-region writes. Idea from the
challenge tree's scratch tokens, re-derived; paired kill-switch A/B: $4/5$
($147.0$--$149.0$ vs $148.3$--$152.7$\,ms). Byte-identity is enforced by a
test that drives a tagged give/take cycle against a fresh generation.
...
was bit-identical on its first full test run --- and measured a
\emph{null} in-prove at both shapes.

The discrepancy exposed the real cost: the in-prove witness bucket was
never builder-bound. The lincheck stripe buffer --- a repacking of $z$
consumed by the lincheck --- was a fresh \emph{zeroed} $128$--$512$\,MB
allocation on every prove, and its first-touch page faults dominated the
bucket. Pooling that one buffer (an untyped byte pool alongside the
field-element pool) took the witness bucket from $18.5$ to $8.0$\,ms at
$n=2^{16}$ and $74$ to $33$\,ms at $n=2^{18}$, both $2/2$ paired ---
more than the packing port's entire predicted value, at $\approx$40
lines. The quad kernel was reverted unmerged; its design survives in the
commit history. This is the third time the campaign's largest available
win sat in the allocation story rather than the kernel (constant-region
elision above, the pooled round-1 precompute, and this): audit where the
buffers come from before porting compute.

\paragraph{Negative space for multithreading.}
...
\multicolumn{2}{@{}l}{PCS open cumulative (\S\ref{sec:composedfold})} &
  \multicolumn{2}{r@{}}{$\approx160 \to \approx145\,$ms\quad($-9\%$)}\\
