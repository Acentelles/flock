...
In progress. The campaign target mirrors the single-threaded closure above:
CPU-only multithreaded throughput within $10\%$ of the challenge tree
(currently $1.19\times$ apart at the $2^{16}$ shape and $1.49\times$ at the
ranked $2^{18}$ shape, where the challenge tree's shape-gated scheduling
stack engages). The first candidate --- round~1's challenge-independent AB
transform overlapped with the commit under \code{rayon::join} --- is
implemented and under measurement as of this revision; subsections will be
added as mechanisms pass their paired tests.
