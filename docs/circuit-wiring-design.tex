...
sequentially-dependent duplex rounds. At L0 that is $\sim$62 sequential
compressions in place of $243{+}$ --- one finalize, one counter-mode XOF
fill whose blocks are mutually independent, one bulk re-absorb. This
directly sizes the FS chain table, which has to replay the whole chain.
A bit-packed squeeze (one field element carrying $\sim$6 query indices)
would cut it by another $\sim$8$\times$ and remains available; it was not
taken because it trades the short low-bit mask for a full 128-bit
decomposition per squeezed element.
