...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Advice publics} — values the circuit consumes but cannot
        cheaply derive (eq-tensor expansions, the zerocheck's skip-round
        seed, per-round $g_0$, deferred matrix evaluations). Each is bound
        by a checker equation over kind-(1) values and proof data.
\end{enumerate}
...
\begin{itemize}[nosep]
  ...
  \item \emph{Advice seeds and chain ends}: the zerocheck seed equals the
        skip-round interpolation at $z$; each published chain end equals
        the native replay from bound data.
  \item \emph{The family-H batch} (boundary-first; the standing upgrade
        ...
        $\F_2$-linear maps of bound words, recomputed natively.
...
contract shrinks monotonically as items move in-circuit (the family-H
batch is one coherent pass of boolean-class bit machinery), and it is
