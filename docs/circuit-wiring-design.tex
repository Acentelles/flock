...
\begin{itemize}[nosep]
  ...
  \item \emph{The family-H batch} (boundary-first; the standing upgrade
        target): the RS transpose dots (a bit-matrix transpose), the
        linearized fold coefficients, and the $V$ recombination's
        Frobenius squarings — $\F_2$-linear maps of bound words,
        recomputed natively. Two items have LEFT the batch, in-circuit:
        \begin{itemize}[nosep]
...
\paragraph{The family-H batch is not one pass — it splits by cell-slot
cost.} The cell space's $c$ bits count \emph{IO words} (one cell-slot
per schema word of each gate type, plus one per $2^\nu$ block of
publics), so a boolean bit-gadget table is charged by its wired words,
not its live rows. That splits the batch:

\begin{itemize}[nosep]
  \item \emph{Element-tractable — both landed}: the skip interpolation
        and the R=2 anchor expect including $\hat g(\rho'')$ (see the
        family-H bullet above). The enabling facts: in characteristic 2
        the eq factor is \emph{affine} ($1+a+b$), so the existing
        \code{PrefixGate} slot evaluates every $\mathrm{eq}(\rho^{2^{-j}},
        \rho'')$ with zero-padding (padding pairs contribute 1); the
        inverse-Frobenius points ride as advice verified by
        \emph{forward} squaring deltas on the existing spine slot;
        $\gamma$-powers are spine MACs. One new cell-slot group
        (\code{AssistLayerGate}, 13 IO words) — $\mu$ held at 21; cost is
        rows ($\approx 128$ prefix chains $+\ 127m$ advice checks) and
        $\approx 2 \cdot 127m$ publics ($\approx$ 20 KiB of proof).
  \item \emph{Cell-slot-expensive}: the RS transpose dots and the $V$
        recombination's Frobenius twists. A faithful transpose table
        needs its 128 input words \emph{wired} (free-witness inputs are
        unsound here — binding the inputs to the stream is the content)
        plus outputs: $\geq 256$ new cell-slots, i.e.\ $c{+}1$ and a
        verifier-cost step, or tiling into $32{\times}32$ blocks
        (64 words) against whatever slot headroom remains. The clean fix
        is per-slot row capacity (the ``jagged remainder''): a
        transpose table with 2 live rows wants $\nu_{\mathrm{slot}} = 1$,
        at which point its index space is trivial. Until that exists,
        these two items \emph{stay boundary} deliberately.
\end{itemize}
