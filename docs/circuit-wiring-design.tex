...
\begin{itemize}[nosep]
  ...
  \item \emph{The family-H batch} (boundary-first; the standing upgrade
        ...
        linearized fold coefficients, the $V$ recombination's Frobenius
        squarings, and the twisted closed form $\hat g(\rho'')$ — all
        $\F_2$-linear maps of bound words, recomputed natively. The
        \emph{skip-round interpolation}, the batch's first item, is
        landed in-circuit: the barycentric \emph{numerator} recurrence
        $A' = A(z{+}\lambda)$, $B' = B(z{+}\lambda) + vA$ over the
        $\Lambda$ nodes needs no inversions and no advice (and is exact
        even at $z$ on a node, where the closed form branches), while the
        $\varphi_8$ structure still pays through the linearized
        $Z_S(z)$ — a 6-step squaring chain — and the shared subspace
        denominators ($\mathrm{den} = c_0$, the formal derivative) baked
        as gate constants. In-circuit, multiplications are cheap and
        inversions are advice: the numerator form beats the
        inverse-based closed form that wins natively.
...
\begin{itemize}[nosep]
  \item \emph{Element-tractable now}: the skip interpolation (landed);
        $\hat g(\rho'')$ and the anchor expect. In characteristic 2 the
        eq factor is \emph{affine} ($1+a+b$), so the existing
        ...
        \emph{forward} squaring deltas $y\cdot y + \mathrm{prev} = 0$ on
        the existing spine slot; $\gamma$-powers are spine MACs. No new
        cell-slots — cost is rows ($\approx 128$ prefix chains $+\ 127m$
        advice checks) and $\approx 2 \cdot 127m$ publics.
