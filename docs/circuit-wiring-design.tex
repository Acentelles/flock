...
\begin{enumerate}[nosep]
  ...
  \item Deferred: count-proportional element prover; LogUp bus;
        capacity-tax refinement; keccak batch-major tier; the in-circuit
        cap-node select (a $c$-bit multiplexer over the absorbed cap
        words --- \code{MerklePathGate} still verifies full-depth paths
        against its synthetic trees).
\end{enumerate}
