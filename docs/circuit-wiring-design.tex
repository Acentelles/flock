...
large-field class and its union integration are \textbf{implemented}; the
wiring layer is designed, its primitive reviewed, and its prerequisites
landed.
\end{abstract}
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Wiring = copy constraints} (\S\ref{sec:wiring};
        \emph{designed}). A permutation $\sigma$ on the cell space of all
...
words outside the schema are internal. \emph{Not yet implemented.}
\end{definition}
...
and $16{,}384$ cells. Cell $(\iota{=}2, j{=}17)$ is ``the $m_0$ input
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Inner-protocol shape normalization} (family J), settled:
        \settled{Ron} query sampling \emph{with replacement} (the
        rejection loop and dedup disappear; soundness re-accounted);
        ...
        shortens by $k$) as the later knob. \settled{Ron} The Merkle/FS
        hash is \textbf{BLAKE3 throughout} --- the $2\times$ circuit-size
        win inverts the native HW-SHA decision; the selectable-hash
        machinery is merged.
  \item \textbf{Succinctness closure}: every verifier component polylog
        in-circuit or delegated. The violations are comb reconstruction
        ($O(\nnz)$, 84\% of verify --- ``the one change without which
        recursion is not worth attempting''), the v1 $\sigma$ evaluation
        ($O(2^\mu)$ --- v2 fixes it), and the \emph{jagged remainder}
        (\S\ref{sec:open}) --- avoidable initially by recursing the
        direct path, where that phase does not exist. The wiring
        layer's own verifier is $O(\mu^2)$ sumcheck replay --- closure-
        safe; the element comb is $O(\nnz(A_0))$ --- trivially safe.
\end{enumerate}
...
opening. Implemented through the element lincheck; the wiring GKR slot is
next.
...
\paragraph{Next.}
\begin{enumerate}[nosep]
  \item \textbf{Wiring milestone}: registry IO schemas, circuit
        representation/validation/digest, the cell space, the wiring GKR
        call and gather recombination with the binding requirements of
        \S\ref{sec:wiring}/\S\ref{sec:statement}, public IO.
        First driving workload: a SHA-256 (or BLAKE3) binary-tree circuit.
...
\begin{enumerate}[nosep]
  ...
  \item Element gate granularity: fuse macro-gates (Horner, dot-product-k,
        sumcheck-round) once the verifier-circuit profile shows where the
        cells concentrate.
