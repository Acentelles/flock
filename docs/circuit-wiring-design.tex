...
\subsection{Re-absorbing squeezed output}\label{sec:reabsorb}

\code{FsChallenger} absorbs each challenge back into the transcript after
squeezing it. \textbf{That was redundant, and it is gone} --- \settled{Ron} \emph{taken
2026-07-31}, worth 26\% of the FS chain.

\paragraph{The argument.} Fiat--Shamir needs two things: every challenge is a
deterministic function of everything absorbed before it, and everything
absorbed afterwards binds to it. Write $S_k$ for the state after the $k$-th
squeeze's \emph{header} is absorbed, so $c_k = \mathrm{XOF}(S_k)$. The first
property holds by construction. The second holds because $c_k$ is
\emph{determined by} $S_k$: a later message is absorbed into a state derived
from $S_k$, so binding to the transcript-so-far already binds to $c_k$.
Re-absorbing contributes $\mathrm{absorb}(S_k, f(S_k))$, which is another
deterministic function of $S_k$ alone --- no entropy enters, and no binding is
added that the state did not already carry.

\paragraph{What is actually required} is that the state \emph{advance} between
squeezes; otherwise two consecutive \code{sample\_f128} calls would return the
same value, the state being unchanged. The squeeze header supplies that, since
it is absorbed before the squeeze. The re-absorb is belt-and-braces on a
mechanism already sufficient.

\paragraph{Provenance.} The pattern is Merlin/STROBE's, where squeezing
\emph{mutates} the state because absorb and squeeze are one operation on the
rate. \code{FsChallenger} uses BLAKE3 as a hash, so \code{finalize\_xof} runs
on a clone and mutates nothing; re-absorbing emulates the sponge. Given the
header absorb, the emulation is unnecessary.

\paragraph{Measured} on the element transcript
(\code{TranscriptShape::blake3\_inventory}, and the model in
\code{tests/transcript\_shape.rs}):

\begin{center}
\begin{tabular}{lrrrrrr}
 & absorbed B & absorb & parents & finalize blk & finalize par & \textbf{rows} \\
\hline
as shipped        & 16{,}960 & 265 & 15 & 108 & 208 & \textbf{698} \\
without re-absorb &  8{,}752 & 136 &  7 & 108 & 162 & \textbf{515} \\
\end{tabular}
\end{center}

\noindent
Absorbed bytes nearly halve, because squeezed output was half the stream. Rows
fall only 26\%: the finalize \emph{count} is fixed by the protocol, and only
the stack depth shrinks --- an estimate of $\sim$40\% assumed otherwise and was
wrong. It also removed the circuit's one circular wire, the re-absorbed
challenge taken from the ROOT row that derived it, so \textbf{the FS chain is
now acyclic}: a challenge is only ever an output, and every \code{m} word is a
transcript value or a constant. \code{StreamWord} lost its \code{Squeezed}
variant accordingly, and \code{Stream::finalize\_after} now carries where the
squeezes fall, since nothing in the stream marks them any more.

\paragraph{The caveat, which is unchanged either way.} Both encodings reveal
$\mathrm{XOF}(S)$ and then keep absorbing into $S$, so both rest on that
leaking nothing useful about future challenges. Removing the re-absorb neither
weakens nor strengthens this; it is the duplex assumption, and it should be
stated wherever the FS transcript's security is written down.

\subsection{Mirroring the verifier, for now}\label{sec:mirror}

\settled{Ron} \textbf{The in-circuit replay is hand-written, mirroring the
native verifier} --- decided 2026-07-31, \emph{with a revisit trigger}, not as
a permanent architecture.

The alternative is to make the replay generic over its value type so the
native verifier and a circuit-emitting backend are one source
(\S\ref{sec:open}). Both were live; the mirror wins \emph{at this scale} and
the decision is recorded because it is the kind that otherwise gets made by
accretion.

\paragraph{Why the mirror, now.} The element PIOP replay measures 133
multiplications and 15 inversions --- roughly 150 element rows with the fused
gate, against the FS chain's 515. Hand-writing that is hours, and it prejudges
nothing structural: the gate inventory is tier 0 either way
(\S\ref{sec:open}), and the operation list a generic backend would need is
already derived.

\paragraph{Why it is not permanent.} A mirror is a second implementation of
the verifier, statement by statement, that must stay in lockstep forever. SP1
made this choice and carries $\sim$9.8k lines of \code{recursion/circuit}
against $\sim$1.5k of native verifier --- and keeps them in sync by convention
and \code{/// Reference:} comments. The ratio is the warning.

\paragraph{Revisit when} the opening enters (MVP-3). Its arithmetic is
$\sim$47k constraints against the replay's 148, so the mirror grows by two
orders of magnitude there, and that is where the SP1 ratio starts to bite.
The measurement that should decide it is how much of the native path's
throughput survives genericity --- the $\sim$1.4\,ms opening is the number at
risk, and it is measurable before committing.

\subsection{Describing circuits: the builder}\label{sec:builder}

\settled{Ron} Circuits are described by a \textbf{builder}, not by
hand-written cell lists. \emph{Design settled 2026-07-31; unbuilt.}

\paragraph{The problem.} \code{Circuit::new(registry, counts, num\_public,
wires)} takes the wiring as raw equivalence classes of
\code{Cell::new(slot, row)}. The witness is built \emph{separately}: in
\code{tests/circuit\_wiring.rs} the \code{Tree} struct assembles
$(h_{\mathrm{in}}, m)$ per gate row in one place while the
\code{Vec<Vec<Cell>>} is assembled in another. Two artifacts that must
agree, with nothing enforcing it --- a mismatch is a silently wrong
\emph{statement}, not a compile error. Three circuits are already built this
way (\code{sha256\_binary\_tree\_circuit}, \code{element\_chain\_circuit},
\code{cross\_class\_hash\_into\_mult}); the recursion work would add a
fourth, larger than all of them.

\paragraph{The principle} is the one that justified the transcript recorder
(\S\ref{sec:replacement-sampling} and \code{transcript\_record}): \emph{one
description, both views}. A gate type carries its constraints and its native
evaluation together, so instantiating a gate emits the row, the wiring and
the witness entry from a single call and they cannot drift.

\begin{verbatim}
    trait GateType {
        fn table(&self) -> TableType;                  // constraints + io_schema
        fn eval(&self, inputs: &[Word]) -> Vec<Word>;  // native execution
    }

    impl CircuitBuilder {
        fn slot(&mut self, g: impl GateType) -> SlotId;
        fn gate(&mut self, s: SlotId, inputs: &[Wire]) -> Vec<Wire>;
        fn public(&mut self, w: Wire);
        fn finish(self) -> (Registry, Circuit, Witness);
    }
\end{verbatim}

\code{gate} allocates a row in the slot, binds the input wires (emitting the
copy constraints), runs \code{eval} to fill that row, and returns wires for
the outputs. \code{counts} falls out of how many rows each slot allocated
rather than being passed in.

\paragraph{Two constraints on the shape.} Row allocation must be
deterministic in instantiation order, because \code{Circuit::digest} is
statement-binding (\S\ref{sec:statement}) and a regenerated circuit must be
\emph{the same} circuit. And \code{Wire} must distinguish ``not yet placed''
from a concrete \code{Cell}, so a value can be consumed before its producer
is emitted --- the FS chain needs this, since a squeezed challenge is
re-absorbed into the transcript that produced it.

\paragraph{Validation is free.} Rebuild the three existing hand-built
circuits through the builder and assert \code{Circuit::digest()} and the
proof bytes are unchanged. If the builder is right, nothing moves --- the
same byte-identity discipline the rest of the repo uses.

\paragraph{What it subsumes.} The Fiat--Shamir chain circuit stops being a
bespoke pipeline and becomes a \emph{client}: recorder $\to$ schedule $\to$
builder calls. The byte-packing glue the FS stream needs (message values land
at arbitrary byte offsets, unlike the tree circuits where 128-bit words align
and wiring is pure copy) is just another \code{GateType}. A future
circuit-emitting backend for the verifier's arithmetic (\S\ref{sec:open}) is
another client. Nothing in the builder is recursion-specific.
...
is unaffordable --- measured at 4.2M constraints / 210 inversions on the
leaf outer); v1 is for native verification. Structured wirings
(chains, complete trees) evaluate in $O(\mu)$ --- the existing
\code{merkle\_path.rs} glue is the degenerate case of this
layer and stays (\code{chain.rs} was retired with the chain product,
2026-08-14).

\paragraph{v2 design sketch (2026-08-03).} The observation that makes v2
cheap: with the merged transport, a preprocessed commitment's opening is
just \emph{more packed-direct claims}, and the transcription work has
already put that entire verification path in-circuit. Concretely:

\begin{enumerate}[nosep]
  \item \textbf{Setup} (statement time): commit the circuit-static data
        --- the $\sigma$ tables the wiring GKR evaluates \emph{and} the
        registry-static comb columns the lincheck delegation needs --- as
        one \emph{preprocessing commitment} under the same PCS. The
        circuit digest covers it; under shape normalisation it is a
        CONSTANT of the recursion shape, absorbed once.
  \item \textbf{Per proof}: the verifier's $\hat s_\sigma(\rho)$ (and the
        comb evaluations) become packed-direct claims against the
        preprocessing commitment --- a second merged opening. The claims
        share row points by construction, so the two-product grouping
        collapses them to a handful of dual values.
  \item \textbf{In-circuit}: the second opening verifies with the SAME
        transcribed machinery (query phase against the preprocessing
        caps, spine, multipoint intake) --- no new gate types. The
        $O(2^\mu)$ evaluation is replaced by a second query-phase
        instance: a few thousand blake3 rows, not millions of
        multiplications.
\end{enumerate}

Open sub-questions for route A: the $\sigma$-table column encoding;
shared vs.\ separate commitment trees; the preprocessing-cap absorb
point.

\paragraph{Route B --- defer $\sigma$ into the accumulator
(recommended).} The cell space is $\mu = \nu + c$ bits, so the
$\sigma$ table reshapes as a $2^{\nu} \times 2^{c}$ matrix and
$\hat s_\sigma(\rho)$ with $\rho$ split into (row, col) coordinates is
\emph{literally a} \code{MatrixClaim} --- the object the accumulator
already folds. The outer circuit emits the GKR endpoint claim as a
$\sigma$-family assertion (the assertion-emission machinery of the
boundary contract, \S\ref{sec:boundary}); the accumulator folds
same-shape $\sigma$ claims $2 \to 1$ at every merge node exactly as it
folds the A/B claims (valid because shape normalisation makes $\sigma$
identical at every internal node --- leaf inners are boolean-only and
emit none); the root discharge evaluates $s_\sigma$ once, natively.
In-circuit cost beyond emission: \emph{zero}. The price is the native
fold, $O(2^{\mu})$ per merge node --- a few ms against a $\sim$60\,ms
per-node budget.

This is also the consistent choice: the comb faced the same
delegate-vs-defer fork and DEFERRAL won (matrix-claim accumulation,
succinct verify $\sim$1.9\,ms), and this section's own constraint ---
one design for both --- then selects route B for $\sigma$ too. Route A
stays the recorded fallback if per-node fold cost or accumulator-state
growth ever bites. Route B makes shape normalisation load-bearing
earlier (same-$\sigma$ folding needs one internal-node shape), which
the closure plan requires regardless.
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Inner-protocol shape normalization} (family J), settled:
        ...
        Merkle openings: \emph{capped paths landed} \settled{Ron},
        skipping the full-depth intermediate. Each tree commits its
        depth-$c$ \emph{cap layer}, $c = \lceil \log_2 q \rceil$ clamped
        to the tree depth, and the cap \emph{is} the commitment: the
        $2^c$ nodes are absorbed into the transcript where the root used
        to be, no root at all (\settled{Ron} --- a root over the cap is
        pointless indirection). Openings are independent per-query paths
        of $d-c$ siblings in sample order, duplicates simply repeated;
        the octopus multi-proof is deleted. At $c = \lceil \log_2 q
        \rceil$ the marginal cap doubling and the $q$ saved siblings per
        level roughly cancel, which prices the whole feature at
        $+24.1$\,KiB at $m{=}26$ Fast while native verifier pair hashes
        \emph{drop} $2396 \to 2157 = \sum_\ell q_\ell (d_\ell -
        c_\ell)$ --- cap verification hashes nothing (equality against
        the absorbed layer), and proof size is now closed-form
        deterministic. Wire format is \code{proof\_io} v7.
...
Nowhere, any longer. While the octopus multi-proof was the transport it
wanted strictly ascending unique leaf positions, so the verifier
collapsed the multiset itself --- and while collapsing it had to
\emph{reject a prover that answered one position with two different
rows}, since only one row per position reached the Merkle check while
every slot fed the induced basis. With capped paths landed (the other
half of family J) that machinery is deleted \emph{natively} as well:
every slot carries its own path against the transcript-fixed cap, so a
duplicate answered with two different rows fails its own path check,
and no sort, dedup, or agreement check exists on either side.
...
% ===========================================================================
\section{The boundary-checker contract}\label{sec:boundary}

The transcription work (MVP-4 through the boolean leaf) settled a uniform
trust structure for what the \emph{outer verifier} checks natively about a
circuit proof's public segment. Every published word is one of two kinds:

\begin{enumerate}[nosep]
  \item \textbf{Circuit-derived wires} — chain squeeze outputs, stream
        words, gate outputs. These are bound by the opening; publishing
        them only \emph{exposes} them.
  \item \textbf{Advice publics} — values the circuit consumes but cannot
        cheaply derive (eq-tensor expansions, per-round $g_0$, deferred
        matrix evaluations). Each is bound by a checker equation over
        kind-(1) values and proof data.
\end{enumerate}

The checker obligations, per verified inner (the leaf inventory; the
pure-element inner is the subset without the RS items):

\begin{itemize}[nosep]
  \item \emph{Cap selects}: per query, mask the published challenge word,
        index the absorbed cap, compare the published terminal digest.
  \item \emph{Eq expansions}: the $\alpha$ weights and the leaf-eval
        hi-group weights are $\mathrm{eq}$-tensor expansions of published
        challenge words, recomputed natively and matched against the
        advice inputs.
  \item \emph{Grinding}: per PoW op, $H(\text{digest} \| \text{nonce})$
        has the required leading zeros (canonical zero nonce at zero-bit
        sites); digest and nonce are kind-(1) wires.
  \item \emph{Zero-deltas}: the published identity deltas (zerocheck
        $g_0$ rows, the multipoint's $T_m = \text{anchor}.v$ and
        $\text{claim} = \text{expect}$, the merged boundary) are zero.
  \item \emph{Chain ends}: each published chain end equals the native
        replay from bound data. (The zerocheck's skip-round seed is no
        longer advice: the interpolation is in-circuit — see below — and
        its \code{rc} output additionally \emph{binds}
        \code{final\_c\_eval}, which the transcript never absorbs; the
        checker holds the published wire against the proof field.)
  \item \emph{The family-H batch}: no native-checker obligation remains.
        For RS claim $k$, the circuit computes
        \[
          h_k=\left\langle
             \operatorname{transpose}(\hat s_{v,k}),
             \operatorname{eq}(r'_k)
          \right\rangle,
          \qquad H_{\rm RS}=\sum_{k=0}^{1}\gamma_k h_k,
        \]
        then derives every inverse-Moore coefficient $c_{k,j}$ from the
        same $r'_k,\gamma_k$ wires and checks
        \[
          V_{\rm RS}=\sum_{k=0}^{1}\sum_{j=0}^{127}
             c_{k,j} A_{k,j}^{2^j}.
        \]
        The packed-direct target terms and scalar-group values are added by
        ordinary MAC chains, the recorded $W$ rounds start from the computed
        target, and the terminal relation is a copy constraint
        $W_{\rm final}=q_{\rm eval}(V_{\rm RS}+V_{\rm groups})$.
        Every operand is an already-bound transcript or proof wire. The
        published target and running value remain only as independent native
        test oracles, not as soundness checks. Two adjacent deferred-algebra
        upgrades are likewise in-circuit:
        \begin{itemize}[nosep]
          \item The \emph{skip-round interpolation}: the barycentric
                \emph{numerator} recurrence $A' = A(z{+}\lambda)$,
                $B' = B(z{+}\lambda) + vA$ over the $\Lambda$ nodes needs
                no inversions and no advice (and is exact even at $z$ on
                a node, where the closed form branches), while the
                $\varphi_8$ structure still pays through the linearized
                $Z_S(z)$ — a 6-step squaring chain — and the shared
                subspace denominators ($\mathrm{den} = c_0$, the formal
                derivative) baked as gate constants. In-circuit,
                multiplications are cheap and inversions are advice: the
                numerator form beats the inverse-based closed form that
                wins natively.
          \item The \emph{R=2 anchor expect}, including
                $\hat g(\rho'')$: the accept $\text{claim} =
                \text{expect}$ is a published zero-delta. The two RS
                statements sit at the \emph{unsquared} claim points (the
                anchor coefficient vectors have only $c_0 \neq 0$),
                located as chain wires; $\hat g(\rho'') = \sum_j
                \gamma^j\,\mathrm{eq}(\rho^{2^{-j}}, \rho'')$ builds from
                127 levels of inverse-Frobenius \emph{advice} bound by
                forward squaring deltas $y \cdot y + \mathrm{prev} = 0$
                (squaring is a bijection in characteristic 2, so the
                deltas pin the advice exactly — no checker equation
                remains), with every eq product on the existing prefix
                slot and the boundary DP on the existing assist-layer
                gate.
        \end{itemize}
  \item \emph{Assertion consumption}: the emitted
        \code{ElementAssertion}/\code{MatrixAssertion} fields (points as
        kind-(1) wires, matrix evaluations as advice bound by the
        lincheck's one-equation final check) feed the accumulator from
        the public segment alone.
  \item \emph{The wiring recombination — a connect since round 4}: for a
        \emph{circuit} child, the two \code{verify\_wiring} closures that
        read the child's publics — $\hat w(\rho) = \sum_\iota
        \mathrm{eq}(\rho_{\mathrm{slot}}, \iota)\, v_\iota$ over the gate
        gathers plus each public slot's row-MLE, and $f = g$ — had
        never appeared in this ledger: they were enforced only by the
        scaffolding-tier native verify inside the tape constructors, a
        recorded soundness-tier gap. Both are now in-circuit: the
        H(publics) region's wires feed 8-lane leaf-eval folds at
        $\rho_{\mathrm{row}}$ with hi-group eq weights from a MAC
        doubling build (rows, not advice — $e \cdot \rho$ and
        $e \cdot (1{+}\rho)$ are each one MAC row), the gathers ride an
        eq-slot-weighted MAC chain over their absorbed pd value wires,
        and the sum \emph{connects} to the absorbed $f$; $f = g$ is a
        second connect. Zero new publics, inputs, or slot types.
\end{itemize}

Two properties make this sound and cheap. Soundness: kind-(2) advice
never floats — every piece closes through an equation whose other
operands are kind-(1) or proof scalars the transcript absorbed. Cost:
the checker's native work is $O(q + \text{rounds} + 128R + P +
\#\mathrm{pow})$ field operations plus tens of hashes — nothing
$O(2^m)$, so the deferral discipline survives the boundary. The
contract shrinks monotonically as items move in-circuit, and it is
part of the statement: a checker that skips an item verifies a weaker
claim.

\paragraph{How family H fits the fixed envelope.}
A monolithic transpose table would indeed spend hundreds of IO words. The
implemented relation instead transposes one $8\times8$ bit tile per row. Its
schema is eight bound input words, one fixed selector word, and eight partial
output words (17 IO words total). There are $2\cdot16\cdot16=512$ live tile
rows per verified child. Linearity lets each partial output enter its
equality-weighted dot product immediately, so the circuit never materializes
the full 128-word transpose as element wires.

The inverse-Moore work uses a GHASH-basis identity checked against the native
matrix at shape construction: if $d_t$ is the trace-dual row, then
$d_t=g_0\eta^t$ for $t\geq7$, with only seven low-index corrections. Every
Frobenius row has the same form after squaring $g_0,\eta$, and the seven
corrections. Thus one prefix product and seven MACs compute each coefficient;
only the eight orbit seeds are fixed publics, while later orbit members are
derived by constrained squaring.

For the $V$ recombination, the two RS values for row $j$ are packed into one
quadratic-extension element and squared $j$ times. The second limb is the
second base-field Frobenius power; the first is the first power plus a known
multiple of the second, removed by one MAC. This costs
$\sum_{j=0}^{127}j=8128$ extension multiplications per child. They first use
the existing narrow extension-MAC slot and spill into the existing Ligerito
spine relation only at the former's physical row limit. No extra element type
is introduced. At the m32 two-child fixed point every slot remains within
$2^{14}$ rows, the cell layout remains 511 gate slots plus one public slot
($\mu=23$), and the recursive proof remains approximately 256 KiB.
...
\begin{enumerate}[nosep]
  \item \textbf{Delegation design} (comb + $\sigma$, one design) --- the
        recursion map's top-priority item. The assist half is now
        DESIGNED AND PROVEN: \texttt{docs/multipoint-twisted-assist.tex}
        (the family-K replacement --- $128K$ claimed dual values, one
        product sumcheck, a closed form, one untwisted anchor; soundness
        $\le (128K{-}1{+}2m)/|\mathbb{F}| + \varepsilon_{assist}$).
  \item \textbf{Merged-transport intake} for packed-direct claims +
        \code{stream\_b} support. [Landed; the unmerged path was removed
        2026-08-02 — the merged transport is the only one, and the
        byte-pinned M6 fixtures are its oracle.]
  \item Deferred: count-proportional element prover; LogUp bus;
        capacity-tax refinement; keccak batch-major tier; an
        \emph{in-circuit} cap-node select (mux or LogUp lookup). MVP-6
        handles capping with the \emph{boundary select} instead: the
        circuit publishes each query's challenge word and terminal
        digest, and the checker indexes the absorbed cap natively ---
        the $c$-bit select costs the circuit nothing, at one extra
        published word per query. Go in-circuit only if that public
        surface ever matters. The composite \code{MerklePathGate}
        stays full-depth as the uncapped differential oracle.
\end{enumerate}
...
\begin{enumerate}[nosep]
  ...
  \item Element gate granularity: \emph{candidate list derived}
        ...
        \settled{Ron} \textbf{Do not fuse: the gate inventory stays at
        \emph{tier 0}} (\code{mult} + \code{linear}, which
        \code{ElementTableBuilder} already provides, so everything above
        decomposes with no new table type). The profile this question was
        waiting on now exists and says the cells are not there: per the
        taxonomy in \S\ref{sec:goals}, hashing is 78--98\% of the circuit
        against $\sim$46k multiplications, so fusing would shave cell count
        inside $\sim$12\% of the trace. The operation list survives as the
        \code{Arith} \emph{API} scoping --- what must become backend-generic
        --- and its granularity is set by \emph{native} throughput (the
        $\sim$1.4\,ms opening's blocked and parallel loops), not by circuit
        cost. Two lists, not one.

        A worked example of the cost asymmetry, kept because the rule
        generalizes: $\varphi_8$ is a field homomorphism, so the interpolation
        node set $V = \varphi_8(\{0..2^m-1\})$ is an $\Ftwo$-subspace and its
        vanishing polynomial $Z_V(X) = \prod_{s \in V}(X+s)$ is
        \emph{linearized} (verified:
        \code{phi8\_node\_sets\_have\_linearized\_vanishing\_polynomials}).
        Hence $L_i(z) = Z_V(z) / ((z + \varphi_8(i))\cdot d_i)$ with $d_i$
        constant --- one additive map and one inverse in place of an
        $O(\ell^2)$ product, collapsing $\sim$20k of the $\sim$46k
        multiplications, and $Z_V$ being $\Ftwo$-linear makes it \emph{free in
        the boolean class}. The right response is to fix the \emph{native}
        implementation, after which a tier-0 backend traces the good algorithm
        with no abstraction hiding the choice. The rule: an implementation
        being sub-millisecond and one-time natively says nothing about its
        shape when arithmetised.
  ...
  \item \textbf{The jagged remainder} (named here; \emph{postponed}). At
        ...
        phase does not exist. [Superseded 2026-08: the shipped recursion
        (the tower, \code{src/tower.rs}) verifies over the merged/union
        transport, not the direct path.]
\end{enumerate}
