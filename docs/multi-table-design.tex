...
(tiny) assist --- a few ms, with Frobenius-sharing available at the
Boolean stage (the suffix rows satisfy
$S^{(j)} = \mathrm{Frob}^j(S^{(0)})$). Still to be done before
building: the batched-sumcheck bookkeeping and a soundness write-up
(standard genre --- $\gamma$-batching over the existing assist
argument), and the $c_j$-extraction detail. The \emph{fat verifier} ---
...
slightly smaller (the $\tilde b$ vector and spot-check go, one batched
assist arrives). Verifier: $\lesssim 1$\,ms under the
Frobenius-batched assist. The core identity is now
test-verified; what remains before building is engineering-scale ---
the batched assist protocol and its (standard-genre) soundness
write-up --- rather than an open question.
