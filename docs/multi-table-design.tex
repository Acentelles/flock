...
\paragraph{What it buys, per tier (honest estimates against the
measured baseline).} Prover: the merged pipeline (dense-domain
sumcheck folds $\approx 3$--$4$\,ms $+$ count-proportional $\Phi$-pass
$\approx 4$--$5$\,ms $+$ Frobenius assist $\approx 2$--$4$\,ms)
replaces the current $12.3 / 23.0 / 44.7$\,ms $+$
$\tilde b$/assist at $\approx 9$--$12$\,ms \textbf{flat}: roughly
break-even at full utilization, $\approx -13$\,ms at the $2\times$
tier, $\approx -35$\,ms at $4\times$ --- the $4\times$-tier total
ratio drops from $\approx 1.35$ to $\approx 1.1$ --- and the
$256$--$512$\,MB $b_{\mathrm{combined}}$ transient disappears at every
tier. Proof: slightly smaller (the $\tilde b$ vector and spot-check
go, one batched assist arrives at unchanged size). Verifier:
$\lesssim 1$\,ms added. The core identity is test-verified; the
remaining gap to buildable is the protocol below plus its soundness
write-up.
