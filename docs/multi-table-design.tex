...
\large Design document --- working draft v0.6}
\author{Flock}
\date{July 27, 2026}
...
Everything in this subsection is a \emph{design sketch} --- analyzed but
unimplemented, with the load-bearing algebra stated so it can be checked,
not yet checked against the implementation.
...
\]
with $(\mathrm{row}, \mathrm{col})$ the jagged unrank. Running the
...
translation. Prover cost becomes count-proportional --- the existing
...
$B((I \otimes M_x)\,T) \ne x\,B(T)$. Three options, in current order of
preference:
\begin{enumerate}[nosep]
  \item \textbf{The fat verifier (leading).} Do not delegate: the
        verifier evaluates $\widehat W(\rho)$ itself by the per-column
        tensor DP --- \emph{no new argument, no new proof bytes, no new
        soundness surface}. Estimated cost: per column and layer, two
        surviving transitions on a $128$-vector-of-$\F$ state
        ($\approx 256$ multiplies for the $\alpha$ slot; the
        $\beta$-side maps are $\F_2$-matrices, applicable by byte
        tables as in ring-switching's own fold) --- order $10^7$
        multiplies $\approx$ a few ms, once per proof, count- and
        capacity-independent but \textbf{linear in the registry's
        column count with a heavy constant} ($\approx 25$K multiplies
        per column, vs.\ $\approx 50$ on the assist path): viable at
        today's $\approx 370$ columns, marginal at $\approx 1000$, not
        viable at a many-type registry. This is the comb's scaling
        family (\S\ref{sec:open}) with a $\approx 500\times$ constant
        --- and, unlike the comb, with \emph{no preprocessing escape}:
        the base-block MLEs are registry-static and committable once,
        but \code{col\_prefix\_sums} is per-proof (count-derived under
        height-$n_t$ stacking), so nothing about the twisted weight can
        be committed at registry time. At registry scale, option 2
        stops being optional. Verification goes
        $\approx 11 \to 13$--$16$\,ms at today's registry; comb
        delegation, already the queued answer if verify time matters,
        would more than offset it.
  \item \textbf{A structured assist.} Exploit the Kronecker form of the
        transitions plus $\alpha$-slot binding to keep a sumcheck over
        some of the variables and handle the $\beta$-slot ones another
        way. Open; would need its own soundness write-up (the genre of
        the rejected yr-weighted assist).
  \item \textbf{Recursive ring-switching.} The $\beta$-slot obstruction
        is bit-structure-versus-field-multiplication --- the exact
        mismatch ring-switching itself resolves --- over a domain of
        only $2(m{+}1)$ bits. Speculative.
\end{enumerate}

\paragraph{What it buys, per tier (estimates against the measured
baseline).} Prover: the ring-switch machinery's $12.3 / 23.0 /
44.7$\,ms collapses to $\approx 6$--$7$\,ms flat --- the $4\times$-tier
total ratio drops from $\approx 1.35$ to $\approx 1.13$, and the
$256$--$512$\,MB $b_{\mathrm{combined}}$ transient disappears. Proof:
slightly smaller (the $\tilde b$ vector and spot-check go). Verifier:
$+2$--$5$\,ms under option 1. The decision gate is exactly that
verifier trade --- and whether the tensor-BP construction and the
$\alpha$/$\beta$-slot analysis survive contact with the implementation,
which is the first thing to check before building anything.
...
driver is missing); the lookup/bus layer (\S\ref{sec:open}); and comb
delegation (item~\ref{item:manytypes}), explicitly deferred until
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{The padded-domain floor / merged reduction.} The one
        ...
        count-proportional, with the $\Phi$-twisted weight evaluation
        as the single open design question.
