...
Status: \textbf{prototype implemented}, side by side with the shipped
jagged transport (no default switch, no wire change; the shipped path is
byte-untouched and its fixtures pass unchanged). The load-bearing
algebra is test-pinned (the Frobenius identity, the linearized-%
coefficient extraction, the batched assist against the brute-force
twisted evaluation), and the full pipeline round-trips with a tamper
matrix on real mixed instances; measured results are folded into the
cost paragraphs below. Adoption --- the polish list, integer-lane
support, and the wire/fixture switchover --- is gated on design review.
...
\paragraph{What it buys, per tier --- measured (prototype) and
estimated (after polish).} The design's central claim is now
\textbf{measured}: on the m30 load, the prototype's prove-time growth
from the right-sized tier to $4\times$ over-capacity is
$+14.1$\,ms, against the jagged path's $+40.7$\,ms --- the merged
transport is near-capacity-free, and the $256$--$512$\,MB
$b_{\mathrm{combined}}$ transient is gone at every tier. The
prototype's \emph{constant} is unoptimized: $+34$\,ms over the shipped
path at the right-sized tier, fully attributed to the polish list ---
the pow2-only commit ($\approx 7$\,ms vs.\ integer lanes; needs the
lane-major eq-basis open), the per-element-unrank weight build (needs
the segmented-cursor treatment), the single eq claim riding the mixed
path's \emph{materializing} combine (fusable --- the eq basis
factors), and the untuned $128K$-statement assist prover (including
its $\approx 150$\,MB suffix-row footprint). With those, the
estimate stands at $\approx 9$--$12$\,ms flat: roughly break-even at
full utilization, $\approx -13$\,ms at $2\times$, $\approx -35$\,ms
at $4\times$. Proof: slightly smaller (the $\tilde b$ vector and
...
implemented as a side-by-side \textbf{prototype}
(\code{open\_batch\_merged}, the Frobenius assist, the
\code{\_merged} union prove/verify pair): end-to-end roundtrip and
tamper matrix on real mixed instances, capacity-freedom measured at
the m30 load ($+14.1$ vs $+40.7$\,ms growth to the $4\times$ tier);
its unoptimized constant and the adoption gate are recorded in
\S\ref{sec:merged}.
...
driver is missing); the lookup/bus layer (\S\ref{sec:open}); the
merged-transport polish + adoption decision (\S\ref{sec:merged}); and
