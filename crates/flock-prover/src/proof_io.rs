...
/// v7 (current) drops the Frobenius assist from the Mixed flavor's merged
/// payload: since fancy jagged (ePrint 2025/917 §6) the dense stack is
/// row-major-within-table and the verifier evaluates the twisted weight
/// `Ŵ(ρ)` itself from the aligned tables, so `MergedOpenProof` no longer
/// carries `frobenius` and the transcript absorbs no assist rounds. The
/// commitment also changes: `q` is the row-major permutation of the same
/// words, so v6 files are rejected.
